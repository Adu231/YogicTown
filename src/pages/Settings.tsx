...
const Settings = ({ hideLayout = false }: SettingsProps) => {
  useScrollTop();
  const { user } = useAuth();
  ...
  const innerContent = (
    <div className={hideLayout ? "max-w-5xl mx-auto py-2" : "max-w-5xl mx-auto px-4 md:px-8 py-10"}>
      {!hideLayout && (
  ...
  if (hideLayout) {
    return <div className="py-2">{innerContent}</div>;
  }
